...
class MaskNet(nn.Module):
    ...
    def forward(self, x, return_feat = False):
        ...
        '''
        m1_ = self.f_conv1(x)
        m1 = m1_ * F.layer_norm(self.relu(h1), h1.size()[1:])
        m2_ = self.f_conv2(m1)
        m2 = m2_ * F.layer_norm(self.relu(h2), h2.size()[1:])
        m3_ = self.f_conv3(m2)
        m3 = m3_ * F.layer_norm(self.relu(h3), h3.size()[1:])
        m4_ = self.f_conv4(m3)
        m4 = m4_ * F.layer_norm(self.relu(h4), h4.size()[1:])
        out = self.avgpool(m4)
        out = out.view(out.size(0), -1)
        y = self.linear(out)
        return y
