...
class MaskNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, nf):
        ...
        self.f_conv1 = self._make_conv2d_layer(3, nf, max_pool=True, padding = 1, layer=1)
        self.f_conv2 = self._make_conv2d_layer(nf*1, nf*2, padding = 1, max_pool=True, layer=2)
        self.f_conv3 = self._make_conv2d_layer(nf*2, nf*4, padding = 1, max_pool=True, layer=3)
        self.f_conv4 = self._make_conv2d_layer(nf*4, nf*8, padding = 1, max_pool=True, layer=4)
        self.relu = nn.ReLU()

    @staticmethod
    def _make_conv2d_layer(in_maps, out_maps, max_pool=False, padding = 1, layer=1):
        ln = nn.LayerNorm([out_maps, 64//(2**layer), 64//(2**layer)])
        print(ln)
    ...
    def forward(self, x, return_feat = False):
        ...
        m1_ = self.f_conv1(x)
        m1 = m1_ * F.layer_norm(h1, h1.size()[1:])
        m2_ = self.f_conv2(m1)
        m2 = m2_ * F.layer_norm(h2, h2.size()[1:])
        m3_ = self.f_conv3(m2)
        m3 = m3_ * F.layer_norm(h3, h3.size()[1:])
        m4_ = self.f_conv4(m3)
        m4 = m4_ * F.layer_norm(h4, h4.size()[1:])
        out = self.avgpool(m4)
        #out = self.avgpool(h4)
