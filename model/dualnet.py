...
class Net(torch.nn.Module):
    ...
    def forward(self, x, t, net = None):
        ...
        output = net(x) if net else self.net(x)
    ...
    def observe(self, x, t, y):
        ...
        if not self.buffer.is_empty():
            self.zero_grad()
            x_, y_, _ = self.buffer.get_data(32)
            x_ = torch.cat([x,x_], 0)
            x1, x2 = self.transforms(x_), self.transforms(x_)
        ...
        for _ in range(self.inner_steps):
            ...
            if t > 0:
                with torch.no_grad():
                    old_logit = self.forward(x,t, self.old_net)
                    loss += 0.05 * F.mse_loss(logit, old_logit)
            loss.backward()
            self.opt.step()
            self.update_schedule(False)
        #if self.epoch == 0:
        tt = torch.Tensor([t] * y.size(0)).long().cuda()   
        #self.buffer.add_data(examples = x, labels = y, task_labels = tt, logits = logit.data)
