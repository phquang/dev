...
class Net(torch.nn.Module):
    ...
    def init_optim(self):
        ...
        self.opt = torch.optim.Adam(self.net.parameters(), lr=self.lr,  weight_decay = 0)
    ...
    def observe(self, x, t, y):
        ...
        for _ in range(self.inner_steps):
            ...
            if not self.buffer.is_empty():
                xx, yy, buff_logits  = self.buffer.get_data(self.bsz * 2)
                x1,x2 = self.transforms(xx), self.transforms(xx)
                loss0 = 0.001 *  self.net.BarlowTwins(x1,x2)
                loss0.backward()
                self.opt.step()
        ...
        if not self.buffer.is_empty():
            xx , yy, buff_logits = self.buffer.get_data(self.bsz)
            pred_ = self.net(xx)
            loss2 += 0.5 * self.bce(pred_, yy)
            loss3 += 0.2 * F.mse_loss(pred_, buff_logits)
        loss = loss1 + loss2 + loss3
        loss.backward()
        self.opt.step()

        #if self.epoch == 0:
        self.buffer.add_data(examples = x, labels = y, logits = pred.data)
        return 0.
