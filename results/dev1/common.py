...
class MaskNet(nn.Module):
    ...
    def forward(self, x, return_feat = False):
        ...
        dc1 = self.get_mask(h1.size())
        dc2 = self.get_mask(h2.size())
        dc3 = self.get_mask(h3.size())
        dc4 = self.get_mask(h4.size())
        h1 = dc1 * F.layer_norm(h1, h1.size()[1:])
        h2 = dc2 * F.layer_norm(h2, h2.size()[1:])
        h3 = dc3 * F.layer_norm(h3, h3.size()[1:])
        h4 = dc4 * F.layer_norm(h4, h4.size()[1:])
        m1_ = self.f_conv1(x)
        m1 = m1_ * h1
        m2_ = self.f_conv2(m1)
        m2 = m2_ * h2
    ...
    def get_mask(self,sz):
        drop_prob = 0.3
        if self.training:
            mask = (1-drop_prob) * torch.bernoulli(torch.randn(1, 1, device='cuda').expand(sz), 1-drop_prob)
        else:
            mask = 1.
        return mask
