...
if __name__ == "__main__":
    ...
    fname = args.model + '_' +  str(args.inner_steps) + '_' + args.train_csv + '_' +  datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    fname += '_' + uid
    ...
    d, download, path = eval(args.train_csv) , True, './temp'
    train_loader = ContinualScenario(d( path, split="train", download=download))
    val_loader = ContinualScenario(d( path, split="val", download=download))
    test_loader = ContinualScenario(d( path, split="test", download=download))
    
    n_outputs, n_tasks = train_loader.nb_classes, train_loader.nb_tasks
    task_mask = []
    for va_set in train_loader:
        task_mask.append(np.unique(va_set._y).tolist())
    ...
    for run in range(args.n_runs):
        ...
        if str(args.model) not in ['ftml']:
            model = model.cuda()
        for task , s_tr in enumerate(train_loader):
            tr_loader = DataLoader(s_tr, batch_size=args.batch_size, shuffle=True)
            print('\n--------------------------------------')
            ...
            for _ in tqdm(range(args.n_epochs), ncols=69):
                #for x, y, t in tqdm(tr_loader, ncols=69):
            ...
            model.eval()
            mode='test'
            for task_t, s_te in enumerate(test_loader):
                te_loader = DataLoader(s_te, batch_size=1024, shuffle=True)
